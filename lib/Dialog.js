...
var Dialog = function Dialog(originalMessage, timeoutValue, timeoutMessage) {
     ...
     */
    var isExplicit = function() {
        return self.explicit;
    }

    var setExplicit = function(explicit) {
    ...
    this.receive = function (msg) {
        ...
        if (!matched) {
            if (isExplicit()) {
                ...
                msg.send("Sorry, I didn't understand your response. Please say one of " + choices.map(function(value) { value.explicit_name }).join(", ") + " to continue.")
                resetDialogTimeout();
            } else {
                self.resetChoices();
                clearTimeout(expiration);
