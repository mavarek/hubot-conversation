...
var Dialog = function Dialog(originalMessage, timeoutValue, timeoutMessage) {
    ...
    this.receive = function (msg) {
        ...
        if (!matched) {
            if (self.isExplicit()) {
                ...
                console.log(choices)
                console.log(choices.map(function(choice) { choice.explicit_name }))
                msg.send("Sorry, I didn't understand your response. Please say one of " + choices.map(function(choice) { choice.explicit_name }).join(", ") + " to continue.")
                resetDialogTimeout();
            } else {
                self.resetChoices();
                clearTimeout(expiration);
     ...
     */
    this.addChoice = function (regex, handler, explicit_name) {
        choices.push({regex: regex, handler: handler, explicit_name: explicit_name});
